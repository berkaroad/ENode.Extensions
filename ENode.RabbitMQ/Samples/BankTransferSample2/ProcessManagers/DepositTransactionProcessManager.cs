...
using BankTransferSample2.Commands;
using BankTransferSample2.Domain;
using ENode.Commanding;
using ENode.Messaging;

namespace BankTransferSample2.ProcessManagers
{
        ...
        IMessageHandler<TransactionPreparationCommittedEvent>               //账户预操作已提交
    {
        ...
        {
            evnt.Items.Add("SagaId", $"Deposit#{evnt.Id}");
        ...
        public async Task HandleAsync(TransactionPreparationAddedEvent evnt)
        {
            ...
            {
                await _commandService.SendAsync(new ConfirmDepositPreparationCommand(evnt.TransactionPreparation.TransactionId)
                {
                    Id = evnt.Id,
                    Items = evnt.Items
                });
        ...
        public async Task HandleAsync(TransactionPreparationCommittedEvent evnt)
        {
            if (evnt.TransactionPreparation.TransactionType == TransactionType.DepositTransaction &&
                evnt.TransactionPreparation.PreparationType == PreparationType.CreditPreparation)
            {
                await _commandService.SendAsync(new ConfirmDepositCommand(evnt.TransactionPreparation.TransactionId)
                {
                    Id = evnt.Id,
                    Items = evnt.Items
                });
